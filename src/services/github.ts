...
export class GitHubService {
  ...
  static async getProjects(): Promise<Project[]> {
    try {
      ...
      const ignoredReposString = process.env.IGNORED_REPOS || process.env.NEXT_PUBLIC_IGNORED_REPOS || '';
      const ignoredRepos = ignoredReposString.split(',').map(r => r.trim().toLowerCase());

      const filteredRepos = repositories.filter(repo => {
        if (repo.archived) return false;
        
        // Verifica se o repositório deve ser ignorado (ex: "luysfernnando/luysfernnando" ou "luysfernnando")
        const fullName = repo.full_name?.toLowerCase() || '';
        const name = repo.name?.toLowerCase() || '';
        
        if (name === this.USERNAME.toLowerCase()) return false;
        
        const isIgnored = ignoredRepos.some(ignored => {
          // Remove barras extras para comparar
          const cleanIgnored = ignored.replace(/^\/+/, '');
          return fullName === cleanIgnored || name === cleanIgnored;
        });
      ...
      const projects = filteredRepos.map(repo => ({
          ...
          featured: !!repo.description, // Repositórios com descrição são destaque
